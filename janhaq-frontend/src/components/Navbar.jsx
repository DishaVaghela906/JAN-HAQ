...
export default function Navbar() {
  ...
  if (isAuthenticated) {
    ...
    const publicWithoutContact = publicLinks.filter(
    ...
    <>
          ...
          >
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-6 py-3 text-center ${linkClasses(
                  link.path
                )}`}
              >
                {link.name}
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200/80 dark:border-gray-700 max-w-sm w-[90%] p-6">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 transition"
              >
                Cancel
              </button>
              <button
