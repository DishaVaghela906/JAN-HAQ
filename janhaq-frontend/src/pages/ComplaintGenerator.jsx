...
const ComplaintGenerator = () => {
    ...
    useEffect(() => {
        const fetchDepts = async () => {
            try {
                ...
                // --- CHANGE: Filter for unique departments based on the 'name' property ---
                const uniqueDeptsMap = new Map();
                depts.forEach(dept => {
                    // Assuming dept object has a 'name' property
                ...
                setDepartments(uniqueDepts); // Set state with unique departments
                // --- END CHANGE ---
                
                // Set first department as default if list is not empty
                if (uniqueDepts.length > 0) {
                    setFormData(prev => ({ ...prev, department: uniqueDepts[0].name }));
                }
            } catch (err) {
                console.error("Error fetching departments:", err);
                setError('Failed to load departments. Please try refreshing.');
    ...
    const handleGenerate = async (e) => {
        ...
        // --- CHANGE: Added !formData.phone and updated error message ---
        if (!formData.department || !formData.originalText || !formData.fullName || !formData.area || !formData.phone) {
            return setError('Please fill in all required fields: Department, Full Name, Phone, Area/Locality, and Complaint Description.');
        }
        // --- END CHANGE ---
                ...
                <div className="grid lg:grid-cols-2 gap-10">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 h-fit sticky top-24 border border-gray-200 dark:border-gray-700">
                        ...
                        <form onSubmit={handleGenerate} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={formData.phone}
                                        onChange={handleChange}
                                        required // --- CHANGE: Added required ---
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500"
                                    />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                >
                                    {departments.length === 0 ? (
                                        ...
                                        // This map now automatically works with the unique list
                                        departments.map(dept => (
                                            <option key={dept.name} value={dept.name}>{dept.name}</option>
                                        ))
                                    )}
                            ...
                            <button
                                type="submit"
                                // --- CHANGE: Added !formData.phone to disabled check ---
                                disabled={generating || loading || !formData.department || !formData.originalText || !formData.fullName || !formData.area || !formData.phone}
                                className="w-full flex justify-center items-center px-4 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition duration-150 disabled:bg-blue-400 dark:disabled:bg-blue-800 disabled:cursor-not-allowed"
                            >
                                {generating ? (
                                    <>
...
const ComplaintGenerator = () => {
    ...
    // --- CHANGE: State to hold all departments and unique cities ---
    ...
    const [formData, setFormData] = useState({
        ...
        phone: '',
        area: '',
        city: '', // --- CHANGE: City will now be selected, not pre-filled ---
        department: '',
        originalText: '',
    });
    const [formalComplaint, setFormalComplaint] = useState(null); // { formalText: string, formalSubject: string }
    ...
    useEffect(() => {
        const fetchDeptsAndCities = async () => {
            try {
                ...
                // --- CHANGE: Extract unique, non-empty cities from all departments ---
    ...
    // --- CHANGE: Fill user data when user object is available ---
    useEffect(() => {
        if (user) {
            setFormData(prev => ({
                ...prev,
    ...
    // --- CHANGE: Get available, unique departments based on selected city ---
    ...
    const handleChange = (e) => {
        ...
        // --- CHANGE: If city changes, reset the department ---
        if (name === 'city') {
            setFormData(prev => ({
                ...prev,
                city: value,
    ...
    const handleGenerate = async (e) => {
        ...
        // --- CHANGE: Added !formData.phone and !formData.city ---
    ...
    const handleSubmitComplaint = async () => {
        ...
        // --- CHANGE: Find the full department object based on *both* name and city ---
        ...
        const complaintData = {
            ...
            area: formData.area,
            city: formData.city, 
            departmentId: selectedDepartment._id, // Send the specific department ID
        ...
        try {
            ...
            setFormData(prev => ({
                ...
                area: '',
                originalText: '',
                // Keep name, email, city, and dept for next complaint?
                // city: '',
                // department: '',
            }));
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 h-fit sticky top-24 border border-gray-200 dark:border-gray-700">
                        ...
                        <form onSubmit={handleGenerate} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {/* --- CHANGE: Replaced text input with a select dropdown for City --- */}
                                <div>
                            ...
                            {/* --- CHANGE: Department dropdown is now filtered by city --- */}
                            <div>
                                <label htmlFor="department" className="block text-sm font-medium mb-1">Select Department</label>
                                <select
                                    id="department"
                            ...
                            <button
                                type="submit"
                                // --- CHANGE: Added !formData.phone and !formData.city to disabled check ---
