...
import { getUser } from '../utils/userDb'; // Import getUser
...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    if (userExists) {
      // 2. If they exist, log them in (which retrieves their full profile)
      login({ email }); 
      navigate('/dashboard');
    } else {
      // 3. If not, show an error
    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen flex items-center justify-center px-6">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-10 w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <input
            type="password"
            placeholder="Password (not validated for this demo)"
            value={password}
